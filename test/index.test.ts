...
import { SweetSS } from "../dist";

describe("css class", () => {
  test("initializes with default values", () => {
    const cssInstance = new SweetSS({ name: "test" });
  ...
  test("initializes with custom prefix", () => {
    const cssInstance = new SweetSS({ name: "test", prefix: "custom-" });
    expect(cssInstance.prefix).toBe("custom-");
  });

  test("initializes with exportMap enabled", () => {
    const cssInstance = new SweetSS({ name: "test", exportMap: true });
